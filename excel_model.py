...
react_prompt_template = ChatPromptTemplate.from_messages(
    [
        ...
        ("assistant", "First, I need to understand what data manipulation is required."),
        ("assistant", "Based on the query and the first five rows of the dataframe, I will generate the appropriate pandas code."),
        ("assistant", "I will remember that the rows are only an example of the dataframe, not the whole dataframe"),
        ("assistant", "Next, I will execute this code and return the result."),
    ]
)

# Function to create the agent with ReAct technique
def create_react_agent(df: pd.DataFrame, llm: ChatOpenAI):
...
def excel_invoke(df: pd.DataFrame, query: str):
    # Create the agent using the ReAct technique
    agent = create_react_agent(df, llm)
